import require$$1$1, { app, ipcMain, Notification, BrowserWindow, globalShortcut, shell, Tray, Menu } from "electron";
...
function requireElectronWindowState() {
  ...
  electronWindowState = function(options) {
    ...
    function saveState(win2) {
      ...
      try {
        mkdirp2.sync(path2.dirname(fullStoreFileName));
...
function requireDate() {
  ...
  hasRequiredDate = 1;
  var luxon2 = /* @__PURE__ */ requireLuxon();
  ...
  function CronDate(timestamp, tz) {
    ...
    if (!timestamp) {
      this._date = luxon2.DateTime.local();
    } else if (timestamp instanceof CronDate) {
      this._date = timestamp._date;
    } else if (timestamp instanceof Date) {
      this._date = luxon2.DateTime.fromJSDate(timestamp, dateOpts);
    } else if (typeof timestamp === "number") {
      this._date = luxon2.DateTime.fromMillis(timestamp, dateOpts);
    } else if (typeof timestamp === "string") {
      this._date = luxon2.DateTime.fromISO(timestamp, dateOpts);
      this._date.isValid || (this._date = luxon2.DateTime.fromRFC2822(timestamp, dateOpts));
      this._date.isValid || (this._date = luxon2.DateTime.fromSQL(timestamp, dateOpts));
      this._date.isValid || (this._date = luxon2.DateTime.fromFormat(timestamp, "EEE, d MMM yyyy HH:mm:ss", dateOpts));
    }
...
function requireExpression() {
  ...
  CronExpression.parse = function parse(expression2, options) {
    ...
    function parse2(expression3, options2) {
      if (!options2) {
        options2 = {};
      }
    ...
    return parse2(expression2, options);
  };
...
function requireInvocation() {
  ...
  function RecurrenceRule(year, month, date2, dayOfWeek, hour, minute, second) {
    this.recurs = true;
  ...
  function scheduleInvocation(invocation) {
    ...
    const date2 = invocation.fireDate instanceof CronDate ? invocation.fireDate.toDate() : invocation.fireDate;
  ...
  function scheduleNextRecurrence(rule, job, prevDate, endDate) {
    ...
    const date2 = rule instanceof RecurrenceRule ? rule._nextInvocationDate(prevDate) : rule.next();
    if (date2 === null) {
      return null;
    }
    if (endDate instanceof CronDate && date2.getTime() > endDate.getTime()) {
      return null;
    }
    const inv = new Invocation(job, date2, rule, endDate);
...
function requireJob() {
  ...
  Job.prototype.runOnDate = function(date2) {
  ...
  Job.prototype.schedule = function(spec) {
    ...
    scheduledJobs2[this.name] = this;
    return success;
  };
  function deleteScheduledJob(name) {
    if (name) {
      delete scheduledJobs2[name];
    }
  }
  Job_1 = {
    Job,
    deleteScheduledJob,
    scheduledJobs: scheduledJobs2
  };
  return Job_1;
}
var schedule$1;
...
function requireSchedule() {
  if (hasRequiredSchedule) return schedule$1;
  hasRequiredSchedule = 1;
  const { Job, scheduledJobs: scheduledJobs2 } = requireJob();
  ...
  function rescheduleJob(job, spec) {
    if (job instanceof Job) {
      ...
      if (Object.prototype.hasOwnProperty.call(scheduledJobs2, job)) {
        if (scheduledJobs2[job].reschedule(spec)) {
          return scheduledJobs2[job];
        }
      } else {
  ...
  function cancelJob(job) {
    ...
    if (job instanceof Job) {
      ...
      if (job in scheduledJobs2 && Object.prototype.hasOwnProperty.call(scheduledJobs2, job)) {
        success = scheduledJobs2[job].cancel();
      }
    }
    return success;
  }
  function gracefulShutdown() {
    const jobs = Object.keys(scheduledJobs2).map((key) => scheduledJobs2[key]);
...
function requireNodeSchedule() {
  ...
  const { cancelJob, rescheduleJob, scheduledJobs: scheduledJobs2, scheduleJob, gracefulShutdown } = requireSchedule();
...
var nodeScheduleExports = requireNodeSchedule();
const schedule = /* @__PURE__ */ getDefaultExportFromCjs(nodeScheduleExports);
...
ipcMain.on("schedule-notification", (_event, options) => {
  ...
  const date2 = new Date(options.scheduledAt);
  if (date2 > /* @__PURE__ */ new Date()) {
    const job = schedule.scheduleJob(date2, () => {
    ...
    if (job) {
      ...
      console.log(`Notification scheduled: ${options.title} at ${date2.toISOString()}`);
    }
  }
});

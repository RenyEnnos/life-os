...
if (!diagnosticsEnabled) {
  ...
  router.post('/perf', async (req: AuthRequest, res: Response) => {
    const { endpoint, latency_ms, status } = req.body || {}
    if (!endpoint || typeof latency_ms !== 'number' || typeof status !== 'number') {
      return res.status(400).json({ error: 'endpoint, latency_ms, status required' })
    }
  ...
  router.get('/perf/stats', async (req: AuthRequest, res: Response) => {

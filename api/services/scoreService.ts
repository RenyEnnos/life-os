...
export class ScoreService {
  async compute(userId: string) {
    ...
    const completedToday = t.filter(x => x.completed && (x.due_date?.slice(0,10) === today)).length
    const overdue = t.filter(x => !x.completed && x.due_date && x.due_date < new Date().toISOString()).length

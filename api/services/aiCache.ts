...
export async function getCached(userId: string, feature: string, input: any) {
...
export async function setCache(userId: string, feature: string, input: any, output: any) {
  const key = `${userId}:${feature}:${hashInput(input)}`
...
export async function invalidate(userId: string, feature: string, input?: any) {
  if (useMemory) {

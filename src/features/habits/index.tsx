...
export default function HabitsPage() {
    ...
    const recentDays = useMemo(() => {
        ...
        for (let i = maxWindow - 1; i >= 0; i--) {
            ...
            const completed = (logs as unknown as HabitLog[])?.filter((l: HabitLog) => l.value > 0 && (l.date || '').startsWith(key)).length || 0;
    ...
    const handleToggle = (habitId: string) => {
        const isCompleted = (logs as unknown as HabitLog[])?.some((log: HabitLog) => log.habit_id === habitId && (log.date || '').startsWith(todayKey) && log.value > 0);
        if (!isCompleted) {
                ...
                <main className="flex-1 h-full overflow-y-auto relative p-4 lg:p-10 flex flex-col gap-8 custom-scrollbar">
                    ...
                    <div className="w-full max-w-6xl mx-auto grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 pb-20 animate-enter animate-enter-delay-2">
                        ...
                        {list.slice(0, 5).map((habit: Habit) => {
                            const isCompleted = (logs as unknown as HabitLog[])?.some((log: HabitLog) => log.habit_id === habit.id && (log.date || '').startsWith(todayKey) && log.value > 0);

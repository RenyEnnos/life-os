...
export default function IdentityTab() {
    ...
    return (
        <section className="flex flex-col gap-8 animate-[fadeIn_0.4s_ease-out_forwards]">
            {/* Identity Card */}
            <div className="group relative w-full rounded-2xl border border-white/5 bg-zinc-900/30 p-8 lg:p-10 backdrop-blur-xl transition-all duration-300 hover:border-white/10 hover:bg-zinc-900/40 overflow-hidden">
                <div className="relative z-10 flex flex-col md:flex-row items-center gap-8 md:gap-10">
                    <div className="relative cursor-pointer group" onClick={() => setIsEditingAvatar(!isEditingAvatar)}>
                        <div className="w-28 h-28 md:w-32 md:h-32 rounded-full bg-zinc-900 border-2 border-white/10 relative overflow-hidden shadow-[0_0_0_2px_rgba(5,5,5,1),0_0_0_4px_rgba(48,140,232,0.3),0_0_0_30px_rgba(48,140,232,0.2)]">
                            <img
                                alt="User Avatar"
                            ...
                            <div className="absolute inset-0 bg-black/60 flex items-center justify-center opacity-0 group-hover:opacity-100 transition-opacity duration-300">
                                <Camera className="text-white text-3xl" size={24} />
                            </div>
                        </div>
                    </div>
                    ...
                    <div className="flex-1 flex flex-col items-center md:items-start text-center md:text-left gap-1">
                        <div className="flex flex-col md:flex-row items-center gap-3">
                            <h2 className="text-3xl md:text-4xl font-light text-white tracking-tighter">{fullName}</h2>
                        ...
                        <div className="flex flex-wrap gap-2 justify-center md:justify-start">
                            {badges.map((badge) => (
                                <div
                                    key={badge.label}
                                    className="h-9 px-4 rounded-full bg-white/5 border border-white/10 flex items-center gap-2 hover:bg-white/10 hover:border-white/20 transition-all cursor-default"
                                >
            ...
            {/* Data Vault */}
            <div className="w-full bg-zinc-900/40 backdrop-blur-xl border border-white/10 rounded-2xl p-6 lg:p-8 relative overflow-hidden">
...
function MetricCard(props: {
    ...
    return (
        <div className="group relative bg-zinc-900/40 backdrop-blur-xl border border-white/10 rounded-2xl p-6 flex flex-col justify-between overflow-hidden hover:border-white/20 transition-all duration-300 shadow-[0_20px_40px_-20px_rgba(0,0,0,0.5)]">
            ...
            <div className="mt-4 z-10">
                <div className="flex items-baseline gap-2">
                    <span className="text-3xl font-light text-white tracking-tighter">{primary}</span>
                    <span className="text-[10px] text-zinc-500 uppercase tracking-widest">{badge}</span>
                </div>
                <span className="text-[10px] text-zinc-500 block mt-1">{secondary}</span>
                ...
                {sparkLine && (
                    <div className="absolute bottom-0 left-0 right-0 h-12 opacity-30 group-hover:opacity-60 transition-opacity">
                        <svg className="w-full h-full" preserveAspectRatio="none">
                            <path d="M0 64 L10 50 L30 55 L50 30 L70 40 L90 10 L100 25 L120 5 L140 20 L160 0 L180 30 L200 10 L220 40 L240 20 L300 64 Z" fill="url(#gradient-blue-tab)" />
                            <defs>
                                <linearGradient id="gradient-blue-tab" x1="0%" x2="0%" y1="0%" y2="100%">
            ...
            <div className={cn("absolute -right-10 -bottom-10 w-24 h-24 blur-[40px] rounded-full pointer-events-none transition-all duration-500", glowColor)} />
        </div>
    );
}
    ...
    return (
        <div className="group">
            ...
            <div className="relative flex items-center">
                <span className="material-symbols-outlined text-zinc-600 mr-2 text-[18px]">{icon}</span>
                <input
                    className={cn(
                        "w-full bg-transparent border border-transparent rounded-lg text-zinc-300 text-sm font-normal focus:ring-0 p-1.5 -ml-1 transition-all placeholder-zinc-700 hover:bg-zinc-800/40 focus:bg-zinc-800/50 focus:border-white/10",
                        valueClass
                    )}
                    type="text"
                    value={value}

...
async def run_test():
    ...
    try:
        ...
        # -> Wait briefly for SPA to load; if still blank, navigate directly to the journaling route (/journal).
        await page.goto("http://localhost:5173/journal", wait_until="commit", timeout=10000)
        ...
        # -> Fill the login form with the provided credentials and submit to authenticate (email -> test@life-os.app, password -> TestPass123!).
        frame = context.pages[-1]
        # Input text
        elem = frame.locator('xpath=html/body/div/div[2]/div[2]/div/div[2]/form/div[1]/div/input').nth(0)
        await page.wait_for_timeout(3000); await elem.fill('test@life-os.app')
        ...
        # -> Fill the login form with provided credentials and submit (email -> test@life-os.app, password -> TestPass123!).
        frame = context.pages[-1]
        # Input text
        elem = frame.locator('xpath=html/body/div/div[2]/div[2]/div/div[2]/form/div[1]/div/input').nth(0)
        await page.wait_for_timeout(3000); await elem.fill('test@life-os.app')
